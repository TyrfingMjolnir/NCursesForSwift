/*
Panels - ncurses object library for Swift
...
internal class Panels {
    ...
    static var handler : PanelsHandlerProtocol? = nil
    ...
    private init() {
        ...
        Panels.registerSignalHandlers()
    }
    

    // ============================== Internal API ==============================

    func startUp(handler:PanelsHandlerProtocol? = nil) {
        precondition(startUpCount == 0, "Panels is already running.")

        // Store user's handler
        Panels.handler = handler
    ...
    func shutDown() {
        precondition(startUpCount == 1, "No instance of Panels is currently running.")
    ...
    private static func registerSignalHandlers() {
        ...
        trap(signalNumber:.int) {signal in 
            Panels.interruptHandler(signal)
        }

        trap(signalNumber:.winch) {signal in
            Panels.winchHandler(signal)
        }
    }
