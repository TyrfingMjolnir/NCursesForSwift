/*
Menus - ncurses object library for Swift
...
internal class Menus {
    ...
    static var handler : MenusHandlerProtocol? = nil
    ...
    private init() {
        ...
        Menus.registerSignalHandlers()
    }
    

    // ============================== Internal API ==============================

    func startUp(handler:MenusHandlerProtocol? = nil) {
        precondition(startUpCount == 0, "Menus is already running.")

        // Store user's handler
        Menus.handler = handler
    ...
    func shutDown() {
        precondition(startUpCount == 1, "No instance of Menus is currently running.")
    ...
    private static func registerSignalHandlers() {
        ...
        trap(signalNumber:.int) {signal in 
            Menus.interruptHandler(signal)
        }

        trap(signalNumber:.winch) {signal in
            Menus.winchHandler(signal)
        }
    }
