/*
Forms - ncurses object library for Swift
...
internal class Forms {
    ...
    static var handler : FormsHandlerProtocol? = nil
    ...
    private init() {
        ...
        Forms.registerSignalHandlers()
    }
    

    // ============================== Internal API ==============================

    func startUp(handler:FormsHandlerProtocol? = nil) {
        precondition(startUpCount == 0, "Forms is already running.")

        // Store user's handler
        Forms.handler = handler
    ...
    func shutDown() {
        precondition(startUpCount == 1, "No instance of Forms is currently running.")
    ...
    private static func registerSignalHandlers() {
        ...
        trap(signalNumber:.int) {signal in 
            Forms.interruptHandler(signal)
        }

        trap(signalNumber:.winch) {signal in
            Forms.winchHandler(signal)
        }
    }
